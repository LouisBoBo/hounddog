...
import com.slxk.hounddog.mvp.ui.activity.LoginActivity;
    ...
    public Response onHttpResultResponse(@Nullable String httpResult, @NonNull Interceptor.Chain chain, @NonNull Response response) {
        ...
        if (!TextUtils.isEmpty(httpResult) && RequestInterceptor.isJson(response.body().contentType())) {
            try {
                ...
                assert res != null;
                if (Api.SUCCESS != res.getErrcode() && res.getErrcode() != Api.Operational_Restrictions && res.getErrcode() != Api.Mobile_Code_Error){
                ...
                if (Api.OUT_OF_DATE == res.getErrcode() || Api.Operational_Restrictions == res.getErrcode()) {//登录出错
                    ...
                    SPUtils.getInstance().remove(ConstantValue.Family_Sid_Login);
