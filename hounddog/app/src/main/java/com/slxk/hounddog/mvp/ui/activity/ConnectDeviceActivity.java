...
public class ConnectDeviceActivity extends BaseActivity<ConnectDevicePresenter> implements ConnectDeviceContract.View {
    ...
    private boolean blue_newest_version = false;// 蓝牙固件是否是最新版本
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent != null) {
                ...
                if (type == 3) {
                    ...
                    onBluetoothUpgradeParse(receive, device);
                } else if (type == 4) {
                    BleDevice device = intent.getParcelableExtra("device");
                    if(device.getName() != null && device.getName().length() > 3) {
                        String device_number = device.getName().substring(3); // 接收机设备号
                        onShowProgress(device_number);
                    }
                } else if (type == 5) {
                    ...
                    if(device.getName() != null && device.getName().length() > 3) {
                        String device_number = device.getName().substring(3); // 接收机设备号
                        onBluetoothPackageIndexParse(receive, device_number);
                    }
                } else {
                    BleDevice device = intent.getParcelableExtra("device");
                    switch (type) {
     ...
     */
    public void onBluetoothPackageIndexParse(String receive, String device_number) {
        ...
        if (index < dataIndexTotal) {
            if (mHandler != null) {
                ...
                onSendUpgradeContent(device_number);
            }
        }
    }
...
//        }
     ...
     */
    public void onBluetoothUpgradeParse(String receive, BleDevice device) {
        LogUtils.e("onBluetoothUpgradeParse=" + receive);
        if (TextUtils.isEmpty(receive)) {
            if(device.getName() != null && device.getName().length() > 3) {
                String device_number = device.getName().substring(3); // 接收机设备号
                mBluetoothManagerUtil.onCheckBluetoothUpgrade(device_number, "00");
            }
            return;
        }
        ...
        if (!TextUtils.isEmpty(upgrade_result)) {
            switch (upgrade_result) {
                case "00":
                    ...
                    break;
                case "02":
                    submitUpgradeResult(upgrade_number, upgrade_result);
    ...
    private void getBluetoothVersion(String device_number, String version_info) {
        ...
        if (version.length == 2) {
            ...
            String url = ConstantValue.getBluetoothUpgradeVersion(device_number, mUpgrade_type, version_info);
            if (getPresenter() != null) {
                getPresenter().getBluetoothUpgradeVersion(url,device_number);
            }
        }
    }
     ...
     */
    private void submitUpgradeResult(String device_number, String result) {
        if (mVersionBean != null) {
            ...
            String url = ConstantValue.submitReportUpgradeRst(device_number, mVersionBean.getData().getTask_id(),
    ...
    @Override
    public void getBluetoothUpgradeVersionSuccess(BluetoothUpgradeVersionBean bluetoothUpgradeVersionBean,String device_number) {
        mUpgradeContent = "";
        ...
        if (bluetoothUpgradeVersionBean.getErrcode() == 0) {
            ...
            if (bluetoothUpgradeVersionBean.getData().getFlag() != 0) {
                if (!TextUtils.isEmpty(blue_version)) {
                    ...
                    if (dirFile.exists()) {
                        // 已存在升级包
                        onStartBluetoothUpgrade(path,device_number);
                    } else {
                        // 未存在升级包
                        onDownloadVersionFile(blue_version, bluetoothUpgradeVersionBean.getData().getUrl(), device_number);
                    }

                } else {
                    onNotUpgradeVersion(device_number);
                }
            } else {
                onNotUpgradeVersion(device_number);
            }
     ...
     * 当前版本已经是最新版本
     */
    private void onNotUpgradeVersion(String device_number) {
        mBluetoothManagerUtil.onCheckBluetoothUpgrade(device_number, "00");
     ...
     * @param url         下载地址
     */
    private void onDownloadVersionFile(String dst_version, String url,String device_number) {
     ...
     * @param path 升级文件本地地址
     */
    private void onStartBluetoothUpgrade(String path, String device_number) {
        try {
            ...
            if (!TextUtils.isEmpty(mUpgradeContent)) {
                mBluetoothManagerUtil.onCheckBluetoothUpgrade(device_number, "02");
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        onSendUpgradeLength(device_number);
                    }
                }, 500);
            }
        } catch (Exception e) {
     ...
     * 开始下发0101指令，下发
     */
    private void onSendUpgradeLength(String device_number) {
        ...
        if (!TextUtils.isEmpty(blue_version)) {
            ...
            mBluetoothManagerUtil.onSendUpgradeMessage(value, device_number);
        }

    }

    private void onShowProgress(String device_number) {
        if (mProgressDialog != null && mProgressDialog.isAdded()) {
            mProgressDialog.dismissAllowingStateLoss();
            mProgressDialog = null;
        }
     ...
     * 发送升级包
     */
    private void onSendUpgradeContent(String device_number) {
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                ++packageIndex;
                onSendValue(device_number);
            }
        }, 150);
    }
     ...
     * 开始传输蓝牙协议
     */
    private void onSendValue(String device_number) {
        if (packageIndex != dataIndexTotal + 1 && mUpgradeContent.length() > 0) { //总包个数时
            String content = "";
            ...
            mBluetoothManagerUtil.onSendUpgradeContent(content, device_number, packageIndex);
            if (mProgressDialog != null && mProgressDialog.isAdded()) {
                if (packageIndex == dataIndexTotal) { //总包-1，最后一个包
            ...
            mBluetoothManagerUtil.onSendUpgradeMessage("0301010000", device_number);
            if (mProgressDialog != null && mProgressDialog.isAdded()) {
                mProgressDialog.dismissAllowingStateLoss();
            }
            mProgressDialog = null;
